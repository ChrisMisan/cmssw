...
//----------------------------------------------------------------------------------------------------
CTPPSDiamondDQMSource::PotPlots::PotPlots(DQMStore::IBooker& ibooker, unsigned int id,unsigned int windowsNum, bool plotOnline, bool plotOffline)
    : HitCounter(0),
      MHCounter(0),
  ...
  if(plotOffline){
    ...
    trackTimeVsBX=ibooker.book2D("track time vs BX",title+" track time vs LS;BX;track_time(ns)",4000,0,4000, 500, -25, 25);
    trackTimeVsXAngle=ibooker.book2D("track time vs xangle",title+" track time vs xangle;xangle;track_time(ns)",60,120,180,500, -25, 25);
    ...
    trackTimeVsBXProfile=ibooker.bookProfile("track time vs BX profile",title+" track time vs LS profile;BX;track_time(ns)", 500, -25, 25,4000,0,4000);
    trackTimeVsXAngleProfile=ibooker.bookProfile("track time vs xangle profile",title+" track time vs xangle profile;xangle;track_time(ns)",500, -25, 25,60,120,180);
    ibooker.setCurrentFolder(path);  
  }

  for(unsigned int i=0;i<windowsNum;i++){
    std::string window=std::to_string(i*25)+"-"+std::to_string((i+1)*25);
    activity_per_bx[i] =ibooker.book1D("activity per BX "+window, title + " Activity per BX "+window+" ns;Event.BX", 3600, -1.5, 3598. + 0.5);
  ...
  leadingEdgeCumulative_both = ibooker.book1D(
      "leading edge (le and te)", title + " leading edge (le and te) (recHits); leading edge (ns)", 25*windowsNum, 0, 25*windowsNum);
  ...
  leadingEdgeCumulative_le =
      ibooker.book1D("leading edge (le only)", title + " leading edge (le only) (DIGIs); leading edge (ns)", 25*windowsNum, 0, 25*windowsNum);
...
CTPPSDiamondDQMSource::PlanePlots::PlanePlots(DQMStore::IBooker& ibooker, unsigned int id, unsigned int windowsNum)
  ...
  pixelTomography_far =
      ibooker.book2D("tomography pixel", title + " tomography with pixel;x + 25 OOT (mm);y (mm)", 25*windowsNum, 0, 25*windowsNum, 8, 0, 8);
...
CTPPSDiamondDQMSource::ChannelPlots::ChannelPlots(DQMStore::IBooker& ibooker, unsigned int id,unsigned int windowsNum)
  ...
  leadingEdgeCumulative_both =
      ibooker.book1D("leading edge (le and te)", title + " leading edge (recHits); leading edge (ns)", 25*windowsNum, 0, 25*windowsNum);
  leadingEdgeCumulative_le =
      ibooker.book1D("leading edge (le only)", title + " leading edge (DIGIs); leading edge (ns)", 25*windowsNum, 0, 25*windowsNum);
  trailingEdgeCumulative_te = ibooker.book1D(
      "trailing edge (te only)", title + " trailing edge (te only) (DIGIs); trailing edge (ns)", 25*windowsNum, 0, 25*windowsNum);
  ...
  pixelTomography_far =
      ibooker.book2D("tomography pixel", "tomography with pixel;x + 25 OOT (mm);y (mm)", 25*windowsNum, 0, 25*windowsNum, 8, 0, 8);
...
void CTPPSDiamondDQMSource::analyze(const edm::Event& event, const edm::EventSetup& iSetup) {
  ...
  if(plotOffline_)
    for (const auto& tracks : *diamondLocalTracks) {
      ...
      for (const auto& track : tracks) {
        ...
        int startBin_near = trackHistoInTimeTmp->FindBin(track.x0() - diamShifts_[detId_near.rpId()].global - track.x0Sigma());
        ...
        for(const auto& tracks_far : *diamondLocalTracks){
          ...
          for (const auto& track_far : tracks_far) {
            ...
            int startBin_far = trackHistoTmpYAxis->FindBin(track_far.x0() - diamShifts_[detId_far.rpId()].global - track_far.x0Sigma());
            ...
            for (int i = 0; i < numOfBins_near; ++i) 
              for (int y = 0; y < numOfBins_far; ++y){ 
                trackHistoTmp->Fill(trackHistoTmpXAxis->GetBinCenter(startBin_near + i),trackHistoTmpYAxis->GetBinCenter(startBin_far+y));
                if(tracks.size()<3&&tracks_far.size()<trackCorrelationThreshold_)
                  sectorPlots_[detId_far.armId()].trackCorrelationLowMultiplicity->Fill(trackHistoTmpXAxis->GetBinCenter(startBin_near + i),trackHistoTmpYAxis->GetBinCenter(startBin_far+y));
              }
          }
        }
      }
  ...
  for (const auto& rechits : *diamondRecHits) {
    ...
    for (const auto& rechit : rechits) {
      ...
      if(plotOffline_){
        potPlots_[detId_pot].TOTVsLS->Fill(event.getLuminosityBlock().index(),rechit.toT());
        potPlots_[detId_pot].trackTimeVsLS->Fill(event.getLuminosityBlock().index(),rechit.time());
        potPlots_[detId_pot].trackTimeVsBX->Fill(event.bunchCrossing(),rechit.time());
        potPlots_[detId_pot].trackTimeVsXAngle->Fill(hLhcInfo->crossingAngle(),rechit.time());

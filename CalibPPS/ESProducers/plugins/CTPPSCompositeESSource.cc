...
CTPPSCompositeESSource::CTPPSCompositeESSource(const edm::ParameterSet &conf)
  ...
  if(ctppsRPAlignmentCorrectionsDataXMLpSet.getParameter<std::vector<std::string> >("MisalignedFiles").empty())
    ...
    auto root = std::make_shared<DetGeomDesc>(&fv);
    ctppsGeometryESModuleCommon->buildDetGeomDesc(&fv, root.get());
  
    pData.idealGD= root;
  
    DetGeomDesc* misalignedGD = nullptr;
    ctppsGeometryESModuleCommon->applyAlignments(*root, pData.acMisaligned.get(), misalignedGD);
    pData.misalignedTG=std::make_shared<CTPPSGeometry>(misalignedGD);


    DetGeomDesc* realGD = nullptr;
    ctppsGeometryESModuleCommon->applyAlignments(*root, pData.acReal.get(), realGD);
    pData.realTG=std::make_shared<CTPPSGeometry>(realGD);
